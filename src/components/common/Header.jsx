  ...
  return (
    <>
      <header className="z-50 header sticky top-0 bg-white shadow-md flex items-center justify-between py-2 sm:py-0 sm:px-8">
        {/* logo */}
        <Link to="/cacharreria_cosas_bonitas/" className="w-1/2 sm:w-2/12 flex-shrink-0">
          <img src={Logo} alt="logo" className="hidden p-2 md:block flex-shrink-0"/>
          <img src={LogoSmall} alt="logo small" className="block w-1/5 m-auto p-1 sm:w-1/2 md:hidden flex-shrink-0"/>
        </Link>
        {/* navigation */}
        <nav className={`sm:block ${isOpen ? "block" : "hidden"} absolute sm:relative w-full sm:w-auto bg-white sm:bg-transparent z-20 sm:z-0 top-full sm:top-auto left-0 sm:left-auto font-semibold`}>          
        ...
        <div className="m-auto sm:hidden">
          <BurgerMenuIcon onClick={() => setIsOpen(!isOpen)} className="p-1.5 cursor-pointer" width={42} height={42} color="black"/>
        </div>
        {/* searchButton */}
